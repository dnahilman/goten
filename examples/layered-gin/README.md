...
├── internal/
│   ├── auth/auth_models.go         ← generated by `goten generate` (package authmodels)
...
├── goten.config.yaml               ← CLI config (plugins + generate output)
...
# 3. Generate Goten's GORM models (core + username plugin) → ./internal/auth
#    (committed in this example; re-run after changing plugins)
goten generate

# 4. Run the server — startup calls db.AutoMigrate(authmodels.AllModels()... + UserProfile)
go run ./cmd/server
```
...
- **`AutoMigrate` for schema changes.** Goten's tables come from generated models + `AutoMigrate` (additive only — no drops/renames/rollback). Destructive changes or data migrations need hand-written SQL applied with your own tooling.
